...
<body class="bg-gray-50">
    ...
    <script>
        ...
        function displayRelayPoints(points) {
            ...
            if (points.length === 0) {
                resultsDiv.innerHTML = '<p class="text-gray-600 text-center py-8">Aucun point relais trouvé dans cette zone.</p>';
                return;
            }

            let html = '<h3 class="font-semibold mb-4 text-gray-800">📍 Points relais disponibles (' + points.length + ')</h3>';
            
            points.forEach((point, index) => {
                html += `
                    <div class="border border-gray-200 rounded-lg p-4 hover:bg-gray-50 cursor-pointer relay-point" 
                         onclick="selectRelay('${point.id}', ${index})">
                        <div class="flex justify-between items-start">
                            <div class="flex-1">
                                <h4 class="font-semibold text-lg text-blue-800">${point.name}</h4>
                        ...
                        <div class="mt-3 grid grid-cols-2 gap-4 text-xs">
                            <div>
                                <h5 class="font-medium text-gray-700">Horaires d'ouverture</h5>
                                <div class="text-gray-600">
                                    ${formatOpeningHours(point.opening_hours)}
                                </div>
                            </div>
                            <div>
                                <button class="w-full bg-blue-600 text-white px-3 py-1 rounded hover:bg-blue-700">
